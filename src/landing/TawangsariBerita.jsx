...
import BeritaIsi from "../assets/beritaisi.png"; // Jika tidak ada gambar default, Anda bisa menghapus ini
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Urutkan data berdasarkan tanggal terbaru dan ambil 3 data pertama
                ...
                {beritaData.map((item) => (
                    <div key={item.uuid} className="flex flex-col items-center">
                        <Link to={`/berita/${item.uuid}`}>
                            <div className="group relative w-96 bg-white border border-gray-200 rounded-lg overflow-hidden shadow-md cursor-pointer">
                                <img className="w-full h-48 md:h-64 lg:h-80 object-cover overflow-hidden transition-transform duration-500 ease-in-out transform group-hover:scale-110" src={`https://tawangsari.com/api/${item.image_path}`} alt={item.title} />
                        ...
                        <p className='pt-4 font-extralight text-xs md:text-sm lg:text-base'>Oleh <strong>{item.uploaded_by}</strong> | {new Date(item.created_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
                    </div>
                ))}
            </div>
